<?php include "head.php"; ?>

<body class="stretched">

    <!-- Document Wrapper
	============================================= -->
    <div id="wrapper" class="clearfix">

        <?php include "header.php" ?>

            
        <div class="section parallax mb-0 mt-0 d-flex align-items-center estandares-laborales portada">
            <div class="container clearfix">
           <div class="arr-der">
                    <img id="portada-mundito" src="images/portada_mundito.gif" width="100%">
                </div>
                <div class="arr-izk">
                    <img id="portada-blanco" src="images/portada_blanco.gif" width="100%">
                </div>
                <div>
                    <h2 class="titulo-seccion" style="color: #fff;">Estándares Laborales</h2>
                </div>
            </div>
        </div>

        <!-- Estándares Laborales
		============================================= -->
        <section id="estandares-laborales">
            <div class="content-wrap">
                <div class="container clearfix">
              

                    <div class="col-12 ml-1">
                        <div class="row">
                            <div class="col-1 p-0"><a href="https://www.un.org/sustainabledevelopment/es/health/"
                                    target="_blank"><img src="images/ods/03.png" alt="Image"></a></div>
                            <div class="col-1 p-0"><a href="https://www.un.org/sustainabledevelopment/es/education/"
                                    target="_blank"><img src="images/ods/04.png" alt="Image"></a></div>
                            <div class="col-1 p-0"><a
                                    href="https://www.un.org/sustainabledevelopment/es/gender-equality/"
                                    target="_blank"><img src="images/ods/05.png" alt="Image"></a></div>
                            <div class="col-1 p-0"><a
                                    href="https://www.un.org/sustainabledevelopment/es/economic-growth/"
                                    target="_blank"><img src="images/ods/08.png" alt="Image"></a></div>

                        </div>
                    </div>
                    <div class="row  text-justify mt-4">
                        <section id="consejo-directivo">
                            <div class="col-12 mt-4">

                                <p>
                                    El personal que conforma el equipo de colaboradores de CAME está integrado por
                                    <strong>90 personas,</strong> quienes se caracterizan por la diversidad, el
                                    profesionalismo y la capacidad de adaptación y respuesta, trabajando diariamente con
                                    <strong>alto grado de compromiso y predisposición</strong> para llevar adelante el
                                    cumplimiento de los objetivos propuestos y añadir valor a nuestra institución.
                                </p>

                            </div>
                        </section>
                        <section id="certificacion-iso">
                            <div class="col-12 mt-4">
                                <h3 class="subtitulo--magical estandares-laborales">Compromiso, bienestar y equipo en medio del Covid-19</h3>
                                <p>Para fomentar el compromiso, bienestar y el trabajo en equipo en medio de las nuevas
                                    condiciones que se presentaron tras la pandemia, implementamos diversas acciones:
                                </p>
                                <div class="col-12">
                                    <div class="row">
                                        <div class="col-3">Encuesta de Relevamiento de Diagnóstico de Personal: la cual
                                            nos
                                            permitió detectar y atender diferentes situaciones y necesidades.
                                        </div>
                                        <div class="col-3">Presentación de resultados de la encuesta de Clima 2019 y
                                            planes
                                            de acción. </div>
                                        <div class="col-3">Implementación de informes sectoriales para identificar
                                            objetivos,
                                            tareas y readecuaciones necesarias.</div>
                                        <div class="col-3">Trivias de preguntas y respuestas: mediante una iniciativa del
                                            sector de Eventos y Multimedia se llevaron a cabo espacios de recreación
                                            virtual
                                            e interacción social.</div>
                                        <div class="col-3">Creación de canal de YouTube “Humanos con recursos” en línea
                                            con
                                            los otros canales de difusión y comunicación al personal como la cuenta de
                                            Twitter.</div>
                                        <div class="col-3">Convenio con el YMCA para mantener clases de gimnasia y
                                            bienestar
                                            personal de manera virtual mediante el mencionado canal de YouTube y redes
                                            sociales.
                                        </div>
                                        <div class="col-3">Capacitaciones y difusión al personal para profundizar los
                                            conocimientos en relación a protocolos de prevención y medidas adoptadas en
                                            la
                                            entidad para prevenir la Covid-19.</div>
                                        <div class="col-3">Participación en acciones solidarias y sustentables: Copa
                                            Garrahan, Mochi Solidaria y Oficina Verde, entre otras.</div>
                                    </div>
                                </div>
                            </div>
                        </section>
                        <section id="codigo-etica">
                            <div class="col-12 mt-4">
                                <h3 class="subtitulo--magical estandares-laborales">Generación de empleo, inclusión y equidad</h3>
                                <table class="table table-hover">
                                    <thead>
                                        <tr>
                                            <th>Niveles de rotación </th>
                                            <th>2020</th>
                                            <th>2019</th>
                                            <th>2018</th>
                                            <th>2017</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td>Índice de rotación total de personal</td>
                                            <td>1,11 %</td>
                                            <td>2,76 %</td>
                                            <td>10,64 %</td>
                                            <td>17,92 %</td>
                                        </tr>
                                        <tr>
                                            <td>Índice de Ingresos</td>
                                            <td>1 %</td>
                                            <td>2 %</td>
                                            <td>7 %</td>
                                            <td>9 %</td>
                                        </tr>
                                    </tbody>
                                </table>
                                <h3 class="subtitulo--magical estandares-laborales">2020</h3>
                                <table class="table table-hover">
                                    <thead>
                                        <tr>
                                            <th>Género</th>
                                            <th>Rango Etario</th>
                                            <th>Cantidad</th>
                                            <th>% Edad / Total</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td>Femenino</td>
                                            <td>20 - 30</td>
                                            <td>5</td>
                                            <td>6 %</td>
                                        </tr>
                                        <tr>
                                            <td></td>
                                            <td>30 - 40</td>
                                            <td>33</td>
                                            <td>37 %</td>
                                        </tr>
                                        <tr>
                                            <td></td>
                                            <td>40 - 50</td>
                                            <td>14</td>
                                            <td>16 %</td>
                                        </tr>
                                        <tr>
                                            <td></td>
                                            <td>50 - 60</td>
                                            <td>3</td>
                                            <td>3 %</td>
                                        </tr>

                                        <tr>
                                            <td>Masculino</td>
                                            <td>20 - 30</td>
                                            <td>4</td>
                                            <td>4 %</td>
                                        </tr>
                                        <tr>
                                            <td></td>
                                            <td>30 - 40</td>
                                            <td>16</td>
                                            <td>18 %</td>
                                        </tr>
                                        <tr>
                                            <td></td>
                                            <td>40 - 50</td>
                                            <td>8</td>
                                            <td>9 %</td>
                                        </tr>
                                        <tr>
                                            <td></td>
                                            <td>50 - 60</td>
                                            <td>4</td>
                                            <td>4 %</td>
                                        </tr>
                                        <tr>
                                            <td></td>
                                            <td>60 - 70</td>
                                            <td>3</td>
                                            <td>3 %</td>
                                        </tr>
                                    </tbody>
                                </table>

                                <table class="table table-hover">
                                    <thead>
                                        <tr>
                                            <th>Nivel de Formación</th>
                                            <th>Cantidad de Empleados 2020 </th>
                                            <th>Cantidad de Empleados 2019 </th>
                                            <th>Cantidad de Empleados 2018 </th>
                                            <th>Cantidad de Empleados 2017 </th>


                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td>Secundario</td>
                                            <td>17</td>
                                            <td>17</td>
                                            <td>25</td>
                                            <td>31</td>
                                        </tr>
                                        <tr>
                                            <td>Terciario</td>
                                            <td>26</td>
                                            <td>32</td>
                                            <td>24</td>
                                            <td>25</td>
                                        </tr>
                                        <tr>
                                            <td>Universitario</td>
                                            <td>47</td>
                                            <td>41</td>
                                            <td>42</td>
                                            <td>41</td>
                                        </tr>
                                        <tr>
                                            <td>TOTAL</td>
                                            <td>90</td>
                                            <td>90</td>
                                            <td>91</td>
                                            <td>97</td>


                                        </tr>

                                    </tbody>
                                </table>
                            </div>
                        </section>
                        <section id="ambiente-laboral">
                            <div class="col-12 mt-4">
                                <h3 class="subtitulo--magical estandares-laborales">Ambiente Laboral y Derechos Humanos</h3>
                                <p>Con el objetivo de acompañar las nuevas necesidades de nuestro personal y continuar
                                    desarrollando las tareas diarias con normalidad, <strong> en 2020 entregamos más de
                                        38 notebooks a los colaboradores,</strong> así como también brindamos
                                    capacitaciones para un manejo óptimo de la nueva herramienta para teletrabajo, Zoom.
                                </p>
                                <p class="center">
                                   <a  style="color:#f39d46;" target="_blank" href="http://www.redcame.org.ar/rse"> Conocé más sobre los beneficios al personal en nuestros anteriores Balances de
                                    Gestión Social y Ambiental</a>  </br>    <a href="http://www.redcame.org.ar/rse" class="button button-border button-rounded button-amber"><i class="icon-book3"></i>Clic aquí</a>
                                </p>

                            </div>
                        </section>
                        <section id="seguridad-trabajo">
                            <div class="col-12 mt-4">
                                <h3 class="subtitulo--magical estandares-laborales">Seguridad en el Trabajo </h3>
                                <p>La Dirección Ejecutiva, y los departamentos de RR.HH., Legales, Intendencia y
                                    Seguridad e Higiene, conformaron el “Comité Covid-19”. En este marco, diseñamos
                                    nuestro Protocolo de Prevención con el objetivo de <strong>minimizar los riesgos de
                                    propagación y contagio del Covid-19 en el lugar de trabajo.</strong> </p>

                                <p>En el mes de julio, adaptamos dicho protocolo a la Norma IRAM EDP 3820, y en agosto
                                    comenzamos el proceso de Certificación de Aplicación Segura de Protocolo Covid-19 y
                                    uso del Sello IRAM. Tras las auditorías correspondientes, en noviembre logramos la
                                    Certificación del Sello IRAM de Protocolo Covid-19 Verificado, una herramienta
                                    estratégica —a la altura de las circunstancias y los desafíos planteados por la
                                    pandemia— que ofrece a toda la comunidad CAME un ambiente de trabajo seguro,
                                    siguiendo los lineamientos y recomendaciones de las entidades y organismos
                                    estatales. </p>

                                <p>Asimismo, trabajamos fuertemente para que nuestras instalaciones cuenten con todos
                                    los recursos necesarios para evitar la propagación, como distribución de artículos
                                    de higiene, señalética y divisores en puestos de trabajo, entre otras
                                    incorporaciones.</p>

                                <p>####(foto de seguridad e higiene) </p>

                            </div>
                        </section>
                        <section id="codigo-etica">
                            <div class="col-12 mt-4">
                                <h3 class="subtitulo--magical estandares-laborales">Desarrollo Humano y Formación
                                </h3>
                                <table class="table table-hover">
                                    <thead>
                                        <tr>
                                            <th>Sector</th>
                                            <th>Becas Otorgadas 2020 </th>
                                            <th>Becas Otorgadas 2019 </th>
                                            <th>Becas Otorgadas 2018</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td>Desarrollo de Carrera</td>
                                            <td>21</td>
                                            <td>19</td>
                                            <td>15</td>
                                        </tr>
                                        <tr>
                                            <td>Diplomatura CAME</td>
                                            <td>5</td>
                                            <td>11</td>
                                            <td>10</td>
                                        </tr>
                                        <tr>
                                            <td>Total General</td>
                                            <td>26</td>
                                            <td>30</td>
                                            <td>25</td>


                                        </tr>
                                    </tbody>
                                </table>


                                <table class="table table-hover">
                                    <thead>
                                        <tr>
                                            <th>Capacitaciones </th>
                                            <th>Horas de Capacitación 2020 </th>
                                            <th>Horas de Capacitación 2019 </th>
                                            <th>Horas de Capacitación 2018 </th>


                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td>Desarrollo de Carrera</td>
                                            <td>1.044</td>
                                            <td>1.401</td>
                                            <td>1.846</td>
                                        </tr>
                                        <tr>
                                            <td>Diplomatura CAME</td>
                                            <td>900</td>
                                            <td>1.980</td>
                                            <td>1.200</td>
                                        </tr>
                                        <tr>
                                            <td>Desarrollo habilidades y competencias</td>
                                            <td>649</td>
                                            <td>927</td>
                                            <td>224</td>
                                        </tr>
                                        <tr>
                                            <td>Seguridad</td>
                                            <td>86</td>
                                            <td>480</td>
                                            <td>312</td>
                                        </tr>
                                        <tr>
                                            <td>Liderazgo</td>
                                            <td>90</td>
                                            <td>123</td>
                                            <td></td>
                                        </tr>
                                        <tr>
                                            <td>Otros</td>
                                            <td></td>
                                            <td>118</td>
                                            <td>116</td>
                                        </tr>
                                        <tr>
                                            <td>Total General</td>
                                            <td>2.769</td>
                                            <td>5.029</td>
                                            <td>3.698</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        </section>



                        <section id="hay-equipo">
                            <div class="col-12 mt-4">
                                <h2 class="underline--magical estandares-laborales">¡Hay Equipo!</h2>
                                <h3 class="subtitulo--magical estandares-laborales">Sumando en CAME</h3>

                                <p>A través de espacios virtuales y talleres para líderes y colaboradores, durante 2020 continuamos fortaleciendo e impulsando el trabajo en equipo por medio del programa interno Sumando en CAME a través del cual dictamos las siguientes capacitaciones:
                                </p>
                                <ul class="ml-4">
                                    <li>Inclusión consciente</li>
                                    <li>Inteligencia emocional y la “nueva normalidad”</li>
                                    <li>Hábitos saludables y alimentación consciente</li>
                                    <li>Ciclo de capacitación a líderes de equipos</li>
                                    <li>Liderazgo y comunicación en tiempos de crisis</li>
                                    <li>Liderazgo inclusivo</li>
                                </ul>
                                <h3 class="subtitulo--magical estandares-laborales">Clima laboral</h3>

                                <p>Difundimos los resultados de la Encuesta de Clima 2019 con todos los colaboradores,
                                    así como también los planes de acción que surgieron a partir del análisis de sus
                                    resultados en los siguientes indicadores:</p>

                                <ul class="ml-4">
                                    <li>Procesos laborales y condiciones de trabajo</li>
                                    <li>Beneficios y reconocimientos</li>
                                    <li>Liderazgo</li>
                                    <li>Desarrollo personal</li>
                                    <li>Valoración de la organización y compromiso</li>
                                </ul>

                            </div>
                        </section>

                        <section id="ODS">
                            <div class="col-12 mt-4">
                                <h3 class="subtitulo--magical estandares-laborales">Estas acciones contribuyen al cumplimiento de los ODS 3, 4, 5 y 8:
                                </h3>
                                <div class="col-12 ml-1 mb-2">
                                    <div class="row">
                                        <div class="col-1 p-0"><a href="https://www.un.org/sustainabledevelopment/es/health/"
                                                target="_blank"><img src="images/ods/03.png" alt="Image"></a></div>
                                        <div class="col-1 p-0"><a href="https://www.un.org/sustainabledevelopment/es/education/"
                                                target="_blank"><img src="images/ods/04.png" alt="Image"></a></div>
                                        <div class="col-1 p-0"><a
                                                href="https://www.un.org/sustainabledevelopment/es/gender-equality/"
                                                target="_blank"><img src="images/ods/05.png" alt="Image"></a></div>
                                        <div class="col-1 p-0"><a
                                                href="https://www.un.org/sustainabledevelopment/es/economic-growth/"
                                                target="_blank"><img src="images/ods/08.png" alt="Image"></a></div>
            
                                    </div>
                                </div>
                                <table class="table table-hover">
                                    <thead>
                                        <tr>
                                            <th>Objetivo</th>
                                            <th>Meta </th>
                                            <th>Indicador </th>
                                            <th>2017</th>
                                            <th>2018</th>
                                            <th>2019</th>
                                            <th>2020</th>
                                        </tr>
                                    </thead>
                                    <tbody>

                                        <tr class="ods-3">
                                            <td> 3</td>
                                            <td> 3.4 De aquí a 2030, reducir en un tercio la mortalidad prematura por
                                                enfermedades no transmisibles mediante su prevención y tratamiento, y
                                                promover la salud mental y el bienestar.</td>
                                            <td> 3.4.1 Colaboradores beneficiados por el convenio con YMCA para
                                                actividades saludables:</td>
                                            <td> 60</td>
                                            <td> 77</td>
                                            <td> 77</td>
                                            <td> 0</td>
                                        </tr>
                                        <tr class="ods-4">
                                            <td> 4</td>
                                            <td> 4.4 De aquí a 2030, aumentar considerablemente el número de jóvenes y
                                                adultos que tienen las competencias necesarias, en particular técnicas y
                                                profesionales, para acceder al empleo, el trabajo decente y el
                                                emprendimiento.</td>
                                            <td> 4.4.1 Horas de capacitación brindadas:</td>
                                            <td> 4.290</td>
                                            <td> 3.698</td>
                                            <td> 5.029</td>
                                            <td> 2.769</td>
                                        </tr>
                                        <tr class="ods-4">
                                            <td> 4</td>
                                            <td> 4b. De aquí a 2020, aumentar considerablemente a nivel mundial el
                                                número de becas disponibles para los países en desarrollo, en particular
                                                los países menos adelantados, los pequeños Estados insulares en
                                                desarrollo y los países africanos, a fin de que sus estudiantes puedan
                                                matricularse en programas de enseñanza superior, incluidos programas de
                                                formación profesional y programas técnicos, científicos, de ingeniería y
                                                de tecnología de la información y las comunicaciones, de países
                                                desarrollados y otros países en desarrollo.</td>
                                            <td> 4.b.1 Colaboradores becados para desarrollo de carrera:</td>
                                            <td> 9</td>
                                            <td> 25</td>
                                            <td> 19</td>
                                            <td> 17</td>
                                        </tr>
                                        <tr class="ods-5">
                                            <td> 5</td>
                                            <td> 5.2 Eliminar todas las formas de violencia contra todas las mujeres y
                                                las niñas en los ámbitos público y privado, incluidas la trata y la
                                                explotación sexual y otros tipos de explotación.</td>
                                            <td> 5.2.2 Proporción de mujeres y niñas de 15 años de edad o más que han
                                                sufrido en los 12 meses anteriores violencia sexual infligida por otra
                                                persona que no sea un compañero íntimo, por edad y lugar del hecho</td>
                                            <td> N/A</td>
                                            <td> 0</td>
                                            <td> 0</td>
                                            <td> 0</td>
                                        </tr>
                                        <tr class="ods-5">
                                            <td> 5</td>
                                            <td> 5.5 Asegurar la participación plena y efectiva de las mujeres y la
                                                igualdad de oportunidades de liderazgo a todos los niveles decisorios de
                                                la vida política, económica y pública.</td>
                                            <td> 5.5.1 Proporción de mujeres respecto al total del personal:</td>
                                            <td> 60%</td>
                                            <td> 63%</td>
                                            <td> 61%</td>
                                            <td> 61%</td>
                                        </tr>
                                        <tr class="ods-8">
                                            <td> 8</td>
                                            <td> 8.6 De aquí a 2020, reducir considerablemente la proporción de jóvenes
                                                que no están empleados y no cursan estudios ni reciben capacitación.
                                            </td>
                                            <td> Proporción de empleados entre 20 y 30 años:</td>
                                            <td> 30%</td>
                                            <td> 20%</td>
                                            <td> 12%</td>
                                            <td> 10%</td>
                                        </tr>
                                        <tr class="ods-8">
                                            <td> 8</td>
                                            <td> 8.8 Proteger los derechos laborales y promover un entorno de trabajo
                                                seguro y sin riesgos para todos los trabajadores, incluidos los
                                                trabajadores migrantes, en particular las mujeres migrantes y las
                                                personas con empleos precarios.</td>
                                            <td> 8.8.1 Tasas de frecuencia de lesiones ocupacionales mortales y no
                                                mortales, desglosadas por sexo y situación migratoria:</td>
                                            <td> 0</td>
                                            <td> 2 casos in itinere</td>
                                            <td> 0</td>
                                            <td> 0</td>
                                        </tr>




                                    </tbody>
                                </table>
                              
                            </div>
                        </section>

                    </div>

                </div>
            </div>


        </section>
        <!-- /Estándares Laborales -->





        <?php include "footer.php" ?>

    </div><!-- #wrapper end -->

    <!-- Go To Top
============================================= -->
    <div id="gotoTop" class="icon-angle-up"></div>

    <!-- JavaScripts
============================================= -->
    <script src="js/jquery.js"></script>
    <script src="js/plugins.min.js"></script>

    <!-- Footer Scripts
============================================= -->
    <script src="js/functions.js"></script>

</body>

</html>